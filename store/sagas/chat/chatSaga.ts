...
import {
	ChatGetConversationsResponseType, ChatGetConversationsType,
...
function* chatGetConversationsSaga() {
	...
	const nextPage: string | null = yield select(getMyConversationsNextPage);
	let page = 1;
	if (nextPage) {
...
function* chatPostMessageSaga(payload: ChatPostMessageType) {
	...
	try {
		if (authSagaContext.tokenType === 'TOKEN' && authSagaContext.initStateToken.access_token !== null) {
			...
			const response: ChatPostMessageResponseType = yield call(() =>
				postFormDataApi(url, authInstance, payloadData),
			);
			// Chat post message has 201 created status
			if (response.status === 201) {
				yield put(setPostMessage(response.data));
				const conversationListindex = conversationsResults.findIndex(
					(item) => item.user_pk === response.data.user,
				);
