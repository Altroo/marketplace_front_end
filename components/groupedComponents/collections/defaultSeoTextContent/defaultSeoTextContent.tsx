...
const DefaultSeoTextContent: React.FC<Props> = (props: Props) => {
	const {title, tags, header, paragraphe, filterMargin} = props;

	return (
		<Stack direction="column" spacing="30px" className={`${Styles.rootStack} ${filterMargin && Styles.filterMargin}`} component="section">
			<Stack direction="column">
				...
				<ThemeProvider theme={ReadOnlyChipsTheme()}>
					<Stack direction="row" flexWrap="wrap" className={Styles.rootChipStack}>
						{tags.map((tag, index) => {
							return <Chip key={index} label={tag} variant="filled" className={Styles.chip} />;
						})}
					</Stack>
				</ThemeProvider>
			</Stack>
